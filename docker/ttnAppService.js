...
class TtnAppService extends appService.AbstractAppService {
	...
	preProcessMessage(mqttTopic, message) {
		...
		let normalizedTopic = mqttTopic;

		//Remove o prefixo de shared subscription:
		//$share/<group>/v3/<appId>/devices/<deviceId>/up
		//vira:
		//v3/<appId>/devices/<deviceId>/up
		if (mqttTopic.startsWith('$share/')) {
			const parts = mqttTopic.split('/');

			if (parts.length < 3) {
				const errorMessage = 'Bad format for a shared TTN topic';
				config.getLogger().error(context, errorMessage);
				return;
			}

			normalizedTopic = parts.slice(2).join('/');
		}

		const splittedMqttTopic = normalizedTopic.split('/');

		//Esperado: v3/<applicationId>/devices/<deviceId>/up
		if (splittedMqttTopic.length !== 5) {
			const errorMessage = 'Bad format for a TTN topic';
			config.getLogger().error(context, errorMessage + ': ' + normalizedTopic);
			return;
		}

		if (
			splittedMqttTopic[0] !== 'v3' ||
			splittedMqttTopic[2] !== 'devices' ||
			splittedMqttTopic[4] !== 'up'
		) {
			const errorMessage = 'Unexpected TTN topic structure';
			config.getLogger().error(context, errorMessage + ': ' + normalizedTopic);
			return;
		}
	...
	observeDevice(devId, devEUI, deviceObject) {
		const mqttTopic = '$share/lora-group/v3/' + this.applicationId + '/devices/' + devId + '/up';
	 ...
	 */
	observeAllDevices() {
		const mqttTopic = '$share/lora-group/v3/' + this.applicationId + '/devices/+/up';
		this.mqttClient.subscribeTopic(mqttTopic);
	}
